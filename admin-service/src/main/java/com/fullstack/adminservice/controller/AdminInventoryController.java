...
public class AdminInventoryController {
    ...
    public ResponseData<List<InventoryItemDto>> list() {
        return new ResponseData<>(
                "SUCCESS", "Lấy danh sách tồn kho thành công", await(inventoryQueryClient.findAll()));
    }

    @GetMapping("/{productId}")
    public ResponseData<InventoryItemDto> get(@PathVariable String productId) {
        return new ResponseData<>(
                "SUCCESS", "Lấy tồn kho thành công", await(inventoryQueryClient.findByProductId(productId)));
    }
    ...
    public ResponseEntity<ResponseData<String>> adjust(
        ...
        try {
            ...
            return ResponseEntity.ok(new ResponseData<>(
                    "SUCCESS", "Cập nhật tồn kho thành công", productId.toString()));
